...
<html>
  ...
  <% 
    ...
    ArrayList <Fournisseur> list = new ArrayList <Fournisseur> ();
     list =(ArrayList <Fournisseur>)request.getAttribute("usernames");
     ArrayList<Fournisseur> statut = new ArrayList<Fournisseur>();
     statut=(ArrayList<Fournisseur>) request.getAttribute("statut");
	 ...
	 <a href="<%= request.getContextPath()%>/BannirFournisseur?username=<%=list.get(	list.size()-1-i).getUsername() %>"><button type="button" class="btn btn-danger">Bannir</button></a> 
  <% } %> 
  						<% if((statut.get(statut.size()-1-i).getEtat())==1) 
  							 {  %>
<a href="<%= request.getContextPath()%>/DebannirFournisseur?username=<%=list.get(	list.size()-1-i).getUsername() %>"><button type="button" class="btn btn-success">Debannir</button></a>  </th>
  <% }  %>
    	<% } %>
                  </thead> 
                  <tbody>
